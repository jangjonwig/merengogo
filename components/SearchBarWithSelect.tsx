...
import type { SearchItem } from '@/lib/types.ts'; // 🔹 변경된 타입

const itemDB: SearchItem[] = [
...
export default function SearchBarWithSelect({ onSelect }: Props) {
  ...
  const handleSelect = (item: SearchItem) => {
    ...
    setShowDropdown(false);
    onSelect(item);
    router.push(`/item/${item.id}`); // ✅ 상세페이지 이동
  };
  ...
  return (
    <div className="w-[320px] relative" ref={containerRef}>
      <input
        ...
        className="w-full h-10 px-8 rounded border border-gray-600 bg-black text-white"
        placeholder="찾으시는 (월코)아이템을 입력하세요."
      />
      ...
      {showDropdown && (
        <div className="absolute top-12 left-0 w-full bg-black border border-gray-700 rounded p-3 flex flex-col text-sm z-10 max-h-60 overflow-y-auto">
          <strong className="text-white text-sm mb-1 block">📋아이템 목록</strong>
          <ul className="space-y-1">
            {previewItems.map((item, index) => (
              ...
              >
                ...
                <span className="text-white">{item.name}</span> {/* ✅ 텍스트 색 밝게 */}
              </li>
            ))}
          </ul>
        </div>
